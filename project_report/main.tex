...
\[
\dot z(t)=f_\theta\!\bigl(z(t)\bigr),\qquad z(0)=z_0,
\]

integrated with \texttt{odeint} (\texttt{torchdiffeq}).
...
We assess predicted velocity maps $\hat{v}$ against ground truth $v$ using four complementary metrics: 
Mean Absolute Error (MAE), Root Mean Square Error (RMSE), Structural Similarity Index (SSIM), and relative $\ell_2$ error. These jointly capture average error, outlier sensitivity, structural fidelity, and normalized magnitude discrepancy.  MAE serves as the primary benchmark in the Kaggle leaderboard. All metrics are computed on the held-out test split. For the top-performing model, we also report MAE on the full \textsc{OpenFWI} dataset. Details and formulae for each metric are provided in Appendix~\ref{appendix:metrics-table}.
